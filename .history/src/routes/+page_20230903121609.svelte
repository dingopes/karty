...
<div class="container mx-auto">
	...
	<div class="grid grid-cols-1 md:grid-cols-1 md:grid-cols-3 gap-4">
	...
	<div class="mt-8 text-center">
		{#if !isScoreZero && isResetGameVisible}
			...
			<div class="m-8 text-xl text-center">
				<p>First with zero is the loser</p>
			</div>
		{:else if isResetGameVisible}
			<button
				class="btn btn-xl bg-gradient-to-br variant-gradient-success-warning text-white"
				on:click={playAgain}
			>
				Hrát znovu
			</button>

			<p class="text-2xl font-bold m-8">
				{#if score1 === 0}
					Vyhral {hrac2}
				{:else}
					Vyhral {hrac1}
